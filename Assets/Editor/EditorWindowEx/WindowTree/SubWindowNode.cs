...
internal class SubWindowNode
{
    ...
    public virtual void DrawGUI(Rect rect, System.Action repaintAction)
    {
        ...
        if (isHorizontal)
        {
            for (int i = 0; i < m_Childs.Count; i++)
            {
                ...
                m_Childs[i].DrawGUI(new Rect(rect.x + offset, rect.y, rect.width * m_Childs[i].weight, rect.height), repaintAction);
        ...
        else
        {
            for (int i = 0; i < m_Childs.Count; i++)
            {
                ...
                m_Childs[i].DrawGUI(new Rect(rect.x, rect.y + offset, rect.width, rect.height * m_Childs[i].weight), repaintAction);
                if (i >= 0 && i < m_Childs.Count)
    ...
    protected virtual bool TriggerAnchorArea(Vector2 position, int depth, SubWindow window, System.Action<SubWindow> preDropAction, System.Action postDropAction)
    {
        ...
        for (int i = 0; i < m_Childs.Count; i++)
        {
            ...
            if (isHorizontal)
            {
                ...
                r = new Rect(rect.x + offset, rect.y + rect.height * 0.8f, rect.width * m_Childs[i].weight, rect.height * 0.2f);
            ...
            else
            {
                ...
                r = new Rect(rect.x + rect.width * 0.8f, rect.y + offset, rect.width * 0.2f, rect.height * m_Childs[i].weight);
                if (r.Contains(position) && m_Childs.Count > 1)
                {
                    if (preDropAction == null)
                        tweenParam = GUIEx.ScaleTweenBox(r, tweenParam, string.Empty, GUIStyleCache.GetStyle("SelectionRect"));
    ...
    private void DropWindow(SubWindow window, System.Action<SubWindow> preDropAction, System.Action postDropAction, bool betweenChilds, int dropIndex)
    {
        if (window == null)
            return;
        if (preDropAction == null)
